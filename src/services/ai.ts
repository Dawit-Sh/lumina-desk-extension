    ...
    output: Output.object({
      schema: z.object({
        ...
        explanations: z.array(z.object({
          original: z.string().describe('The original incorrect phrase or sentence.'),
          correction: z.string().describe('The corrected phrase or sentence.'),
          explanation: z.string().describe('Why it was incorrect and why the new version is better.')
  ...
  const { output } = await generateText({
    ...
    output: Output.object({
      schema: z.object({
        ...
        explanation: z.string().describe('Explanation of how the text was adapted to fit the requested style.')
      })
    }),
  ...
  const { output } = await generateText({
    ...
    output: Output.object({
      schema: z.object({
        ...
        keyPoints: z.array(z.string()).describe('3-5 key bullet points from the text.')
      })
    }),
...
export async function analyzeTone(text: string) {
  const { output } = await generateText({
    ...
    output: Output.object({
      schema: z.object({
        primaryTone: z.string().describe('The single most dominant tone (e.g., Assertive, Joyful, Melancholic, Professional).'),
        secondaryTones: z.array(z.string()).describe('2-3 secondary tones present in the text.'),
        analysis: z.string().describe('A brief explanation of why these tones were identified based on word choice and sentence structure.'),
        formalityScore: z.number().describe('A score from 1 to 10 where 1 is extremely casual/slang and 10 is highly formal/academic.')
      })
    }),
...
export async function processPrompt(
  ...
  if (action === 'make') {
    ...
    schema = z.object({
      resultPrompt: z.string().describe('The detailed, ready-to-use LLM prompt.'),
...
export async function shiftTone(text: string, targetTone: string) {
  const { output } = await generateText({
    ...
    output: Output.object({
      schema: z.object({
        shiftedText: z.string().describe('The text rewritten in the target tone.'),
        explanation: z.string().describe('Explanation of the changes made to achieve the target tone.')
      })
    }),
...
export async function detectAI(text: string) {
  const { output } = await generateText({
    ...
    output: Output.object({
      schema: z.object({
        aiScore: z.number().describe('Estimated probability (0-100) that the text was AI-generated.'),
        detectionAnalysis: z.string().describe('Analysis of why the text was flagged as AI or human.'),
        markers: z.array(z.string()).describe('Specific markers found (e.g., "Uniform sentence length", "High predictability").')
      })
    }),
...
export async function humanizeAndDetect(text: string) {
  const { output } = await generateText({
    ...
    output: Output.object({
      schema: z.object({
        humanizedText: z.string().describe('The text rewritten to sound naturally human.'),
        aiScore: z.number().describe('Estimated probability (0-100) that the original text was AI-generated.'),
        detectionAnalysis: z.string().describe('Analysis of why the original text was flagged as AI or human.'),
        improvements: z.array(z.string()).describe('List of specific creative/humanizing choices made.')
      })
    }),
