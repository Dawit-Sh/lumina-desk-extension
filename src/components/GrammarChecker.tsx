...
export function GrammarChecker({ preserveInformalityDefault, layout }: GrammarCheckerProps) {
  ...
  const [result, setResult] = useState<{
    ...
    explanations: { original: string; correction: string; explanation: string }[];
  } | null>(null);
  ...
  return (
    <div className="flex flex-col gap-8">
      <header>
        <h1 className="text-4xl font-light tracking-tight mb-2 dark:text-white">Grammar Checker</h1>
        <p className="text-gray-500 dark:text-neutral-400">Refine your writing with AI-powered grammar and style corrections.</p>
      </header>
      ...
      <div className={`grid gap-6 ${gridClass}`}>
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-3xl shadow-sm border border-gray-100 dark:border-neutral-800 overflow-hidden flex flex-col transition-colors duration-300">
          ...
          />
          <div className="p-4 border-t border-gray-50 dark:border-neutral-800/50 bg-gray-50/50 dark:bg-neutral-800/30 flex justify-between items-center">
            <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-neutral-400 cursor-pointer hover:text-gray-900 dark:hover:text-neutral-200 transition-colors">
              <input 
                type="checkbox" 
                checked={isInformal} 
                onChange={(e) => setIsInformal(e.target.checked)}
                className="w-4 h-4 rounded border-gray-300 dark:border-neutral-600 text-black dark:text-white focus:ring-black dark:focus:ring-white cursor-pointer bg-white dark:bg-neutral-800"
              />
              Keep it informal
            </label>
            <button
            ...
            >
              {isChecking ? <Loader2 className="animate-spin" size={16} /> : <Sparkles size={16} />}
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-3xl shadow-sm border border-gray-100 dark:border-neutral-800 overflow-hidden flex flex-col min-h-[400px] transition-colors duration-300">
          <div className="p-4 border-b border-gray-50 dark:border-neutral-800/50 bg-gray-50/50 dark:bg-neutral-800/30 flex justify-between items-center">
            <span className="text-sm font-medium text-gray-500 dark:text-neutral-400">Corrected Text</span>
              ...
              {isChecking ? (
                ...
                >
                  ...
                  {result.explanations && result.explanations.length > 0 && (
                    <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-neutral-800">
                      <h3 className="text-sm font-semibold text-gray-900 dark:text-neutral-100 uppercase tracking-wider">Corrections & Explanations</h3>
                      <div className="flex flex-col gap-3">
                        {result.explanations.map((exp, idx) => (
                          <div key={idx} className="bg-gray-50 dark:bg-neutral-800/50 rounded-2xl p-4 text-sm">
                            <div className="flex items-center gap-3 mb-2">
                              <span className="text-red-500 dark:text-red-400 line-through decoration-red-200 dark:decoration-red-900/50">{exp.original}</span>
                              <ArrowRight size={14} className="text-gray-400 dark:text-neutral-500" />
                              <span className="text-green-600 dark:text-green-400 font-medium">{exp.correction}</span>
                            </div>
                            <p className="text-gray-600 dark:text-neutral-400 leading-relaxed">{exp.explanation}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {result.explanations && result.explanations.length === 0 && (
                    <div className="flex items-center gap-2 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 p-4 rounded-2xl">
                      ...
                      <span className="font-medium">Looks good! No major grammar issues found.</span>
                    </div>
                  )}
                </motion.div>
              ) : (
                ...
                >
                  <div className="w-16 h-16 rounded-full bg-gray-50 dark:bg-neutral-800/50 flex items-center justify-center">
                    <AlertCircle size={24} className="text-gray-300 dark:text-neutral-600" />
                  </div>
                  <p className="text-sm">Your corrected text will appear here.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
